...
- Drag/resize (sidebar splitter, note list/detail splitter, and Windowed window move/corner resize) should feel smooth, stable, and 1:1 with shared active styling.
...
- Life Calendar life↔week transitions should stay sidebar-scoped (no full-window flashes) and feel smooth; do not highlight the open note in week-note lists (NoteList already shows selection).
...
- In App Windowed, note-list ellipsis offers Open in new window → session-only `notePopups` floating editors (`NotePopupWindow.svelte`, drag/resize); leaving Windowed closes all popups. Ellipsis Rename inline-edits the title (Enter/blur save, Escape cancel).
